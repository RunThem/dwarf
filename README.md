...
```shell
git clone https://github.com/RunThem/dwarf.git ${your_repository}
cd ${your_repository}

bash -c 'sed -i "s/dwarf/$(basename `pwd`)/g" README.md xmake.lua && git update-ref -d HEAD && git add . && git commit -m "first commit" && git remote remove origin'
